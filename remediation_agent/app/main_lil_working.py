...
def parse_commands_from_text(text: str) -> List[Dict[str, Any]]:
    """Try extracting structured commands from LLM text.

    Strategy:
      1) Look for a JSON array block and parse it.
      2) Otherwise find lines beginning with 'kubectl' and parse them.
    """
    extracted = []

    # 1) JSON block detection (```json [ ... ]``` or raw JSON array)
    import re
    ...
    if m:
        try:
            ...
            if isinstance(arr, list):
                # normalize to expected dict form if needed
                for i, item in enumerate(arr):
                    if isinstance(item, dict):
                        extracted.append({
                            "orig": None,
                            "op": item.get("op") or item.get("action"),
                            "kind": item.get("kind"),
                            "name": item.get("name"),
...
def _parse_kubectl_line(line: str) -> Optional[Dict[str, Any]]:
    """Improved parser for kubectl commands, supports plural forms."""
    try:
    ...
    if cmd == "rollout" and len(parts) > i+2 and parts[i+1] == "restart":
        kind = parts[i+2].rstrip('s')  # handle plural
    ...
    if cmd in ("describe", "get", "delete"):
        kind = parts[i+1].rstrip('s') if len(parts) > i+1 else None  # handle plural
